...
import org.ingservicios.practica1.*;


@Controller
public class HomeController {
	//Vista que cambia el recurso jsp
	final String [] vistas = {"index","registro","admin","bienvenido","compra","carrito"};
	
	//Busca en servlet-content un bean que implemente un dao_usuario_Interface,
	//para poder acceder a la base de datos
	@Autowired
	DaoUsuario_Interface dao_u; 
	
	//Busca en servlet-content un bean que implemente un dao_producto_Interface, 
	//para poder acceder a la base de datos
	@Autowired
	DaoProducto_Interface dao_p;
	
	
	@RequestMapping(value ={"/", "/index"}, method = {RequestMethod.GET,RequestMethod.POST} )
	public String index(HttpServletRequest request, HttpServletResponse response, Model model){
		
		Boolean sicookies = true;
		//Fecha del sistema
    	Date today = new Date();
		...
		//Método que obtiene la sesión de la petición, y si no existe devuelve null
		...
		if(user!=null){
			...
			Boolean existeusuario = dao_u.existsName(user.getNombre());
			...
			if(existeusuario){
				...
				if(user.getAdmin()){
					...
					model.addAttribute("listaUsuario",dao_u.read());
			...
			if(cookies==null || sicookies==false){
				...
				if(request.getParameter("email")==null || request.getParameter("pass")==null){
					...
					String id_pass = request.getParameter("pass");
					
					//Imprimimos por consola del servidor el usuario y la contraseña
					System.out.println("name= "+id_user+" pass= "+id_pass);
					
					//Comprobamos si existe en la base de datos
					Boolean existeUsu = dao_u.existsEmail(id_user);
					...
					if (!existeUsu){
						...
						usuarioDB = dao_u.obtenerUsu(id_user); 
						
						//Si no coincide la clave de la base de datos
						if(!usuarioDB.getClave().equals(id_pass)){
		...
		if(esadmin){
			...
			try{
				...
				Sesion.CrearSession(request, usuarioDB, listaCompra);
	...
	public String registro(HttpServletRequest request,Model model) {
		...
		if(request.getParameter("name")==null|| request.getParameter("pass")==null ||
			...
			if(dao_u.existsName(nombre) || dao_u.existsEmail(email)){
				...
				DtoUsuario usuario = new DtoUsuario (nombre, pass, apellidos, email, direccion, telf);
							
				//Añadimos al usario en la base de datos.
				dao_u.create(usuario);
				
				Sesion.CrearSession(request, usuario, lista);
	...
	public String productos(HttpServletRequest request,Model model) {
		...
		try{
			...
			List<DtoProducto> listaProductos = dao_p.read();
	...
	public String carrito(HttpServletRequest request, Model model) {
		...
		try{
			...
			//Accedemos a la base de datos de productos.
			List<DtoProducto> listaProductos = (ArrayList<DtoProducto>) dao_p.read();
			...
			if(request.getParameter("opcion")!=null){
				...
				if(contador!=0){
					...
					model.addAttribute("lista", dao_p.read());
					model.addAttribute("err",err);
				}
			...
			for(int i=0;i<listaCompra.size();i++){
				...
				while(j<listaProductos.size()){
					...
					if(listaCompra.get(i).getId()==listaProductos.get(j).getId()){
						...
						dao_p.actualizar(listaProductos.get(i).getUnidades()-listaCompra.get(i).getUnidades(), listaProductos.get(j).getId());
	...
	public @ResponseBody String nameDisponible(@RequestParam(value="name",required=true) String idName) {
		...
		boolean existe = dao_u.existsName(idName);
	...
	public @ResponseBody String emailDisponible(@RequestParam(value="email",required=true) String idEmail) {
		...
		//Variable que indica si existe el nombre de usuario
		boolean existe = dao_u.existsEmail(idEmail);
	...
	public String cambiar(HttpServletRequest request, Model model,HttpServletResponse response) {
		...
		try{
			...
			dao_u.actualizar(user,emailAntiguo);
